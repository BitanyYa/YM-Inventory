...
const toggleBtnCls = (active: boolean) =>
  `flex-1 rounded-xl border py-2 text-xs font-semibold transition-colors ${
    active ? 'border-[#0071E3] bg-[#0071E3] text-white' : 'border-[#D2D2D7] bg-white text-[#1D1D1F] hover:border-[#0071E3]/40 dark:border-[#38383A] dark:bg-[#2C2C2E] dark:text-[#F5F5F7]'
  }`;

export const ReturnStockModal: React.FC<Props> = ({ isOpen, product, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    ...
    inventoryService.getProductInventoryDetail(product.id)
      .then((res) => setSoldUnits((res.data.units ?? []).filter((u) => u.status === 'SOLD')))
      .catch(() => setSoldUnits([])).finally(() => setLoadingUnits(false));
  }, [isOpen, product, isSerialized]);

  if (!isOpen || !product) return null;
  const reset = () => { setQuantity(''); setReturnLocation('WAREHOUSE'); setNote(''); setSelectedIds([]); setSoldUnits([]); setError(null); };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try { await inventoryService.returnStock(payload); reset(); onSuccess(); onClose(); }
    catch (err: unknown) { setError((err as { message?: string })?.message || 'Return failed.'); }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto p-4">
      ...
      <div className="relative w-full max-w-md rounded-2xl border border-[#D2D2D7] bg-white shadow-xl dark:border-[#38383A] dark:bg-[#1C1C1E]">
        <div className="flex items-start justify-between border-b border-[#E8E8ED] px-5 py-4 dark:border-[#2C2C2E]">
          <div>
            <h3 className="text-base font-semibold text-[#1D1D1F] dark:text-[#F5F5F7]">Return Stock</h3>
        ...
        <form onSubmit={handleSubmit} className="px-5 py-4 space-y-4">
          <div className="space-y-1.5">
            <span className="text-xs font-semibold text-[#86868B]">Return To *</span>
            <div className="flex gap-2">
              {(['WAREHOUSE', 'SHOP'] as Location[]).map((loc) => (
                <button key={loc} type="button" onClick={() => setReturnLocation(loc)} className={toggleBtnCls(returnLocation === loc)}>{loc}</button>
              ))}
            </div>
          </div>

          {!isSerialized ? (
            <Input label="Quantity *" type="number" min={1} placeholder="e.g. 1" value={quantity} onChange={(e) => setQuantity(e.target.value)} required />
          ) : loadingUnits ? (
            <div className="flex justify-center py-6"><Spinner size="md" /></div>
          ) : soldUnits.length === 0 ? (
            <p className="rounded-xl border border-[#FF9F0A]/30 bg-[#FFF4E0] px-3 py-2.5 text-xs text-[#995E00]">No sold units found for this product.</p>
          ) : (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-[#86868B]">Sold Units ({selectedIds.length}/{soldUnits.length})</span>
                <button type="button" onClick={() => setSelectedIds(selectedIds.length === soldUnits.length ? [] : soldUnits.map((u) => u.id))} className="text-xs font-semibold text-[#0071E3] hover:text-[#0077ED]">
                  {selectedIds.length === soldUnits.length ? 'Deselect All' : 'Select All'}
                </button>
              </div>
