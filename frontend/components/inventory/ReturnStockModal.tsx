    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto">
      ...
      <div className="relative w-full max-w-md overflow-hidden rounded-xl border border-slate-200 bg-white p-6 shadow-xl dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="flex items-start justify-between border-b border-slate-100 pb-4 dark:border-slate-800">
          <div>
            <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100">Return Stock</h3>
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-2 gap-2">
              {(['WAREHOUSE', 'SHOP'] as Location[]).map((loc) => (
                <button key={loc} type="button" onClick={() => setReturnLocation(loc)}
          ...
          {!isSerialized ? (
            <Input label="Quantity *" type="number" min={1} placeholder="e.g. 1"
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold uppercase tracking-wider text-slate-500">
                  Sold Units ({selectedIds.length}/{soldUnits.length})
                </span>
                <button type="button"
                  onClick={() => setSelectedIds(selectedIds.length === soldUnits.length ? [] : soldUnits.map((u) => u.id))}
