...
import { ProductAllocationItem, ReverseAllocationResponse } from '../../types/api';
...
interface ReverseAllocationModalProps {
  isOpen: boolean;
  allocation: ProductAllocationItem | null;
  onClose: () => void;
  onSuccess: (response?: ReverseAllocationResponse) => void;
}

export const ReverseAllocationModal: React.FC<ReverseAllocationModalProps> = ({
  isOpen,
  allocation,
  onClose,
  onSuccess,
  ...
  useEffect(() => {
    if (isOpen && allocation) {
      const remaining = allocation.remainingQuantity ?? (allocation.quantity - (allocation.reversedQuantity ?? 0));
  ...
  const originalQty = allocation.quantity;
  const alreadyReversedQty = allocation.reversedQuantity ?? 0;
  const remainingQty = allocation.remainingQuantity ?? (originalQty - alreadyReversedQty);
  ...
  const handleSubmitReversal = async () => {
    ...
    try {
      const res = await salespersonStockService.reverseAllocation({
      ...
      setShowConfirmation(false);
      // Refresh allocation history in background if requested quantity exceeded state
      onSuccess();
    } finally {
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="mt-0.5 text-xs text-[#6E6E73] dark:text-[#86868B]">
                Return allocated stock from salesperson to Main Shop.
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="rounded-xl border border-[#E8E8ED] bg-[#F5F5F7] p-3.5 space-y-2 dark:border-[#2C2C2E] dark:bg-[#2C2C2E]">
            <div className="flex items-center justify-between text-xs">
              <span className="text-[#6E6E73] dark:text-[#86868B]">Salesperson</span>
              <span className="font-bold text-[#1D1D1F] dark:text-[#F5F5F7] flex items-center gap-1">
                <UserIcon size={13} className="text-[#86868B]" />
                {allocation.salesperson.name}
              </span>
            </div>
            ...
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-bold text-[#1D1D1F] dark:text-[#F5F5F7]">
                {allocation.product.name} ({allocation.product.brand})
              </span>
            </div>
            ...
            <div className="pt-2 border-t border-[#E8E8ED] dark:border-[#38383A] grid grid-cols-3 gap-2 text-center text-xs">
              <div className="rounded-lg bg-white p-2 dark:bg-[#1C1C1E]">
                <span className="block text-[10px] text-[#86868B]">Allocated</span>
                <span className="font-bold text-[#1D1D1F] dark:text-[#F5F5F7]">
                  {originalQty}
                </span>
              </div>
            ...
            <form onSubmit={handleProceedToConfirm} className="space-y-4">
              ...
              <div>
                ...
                {!isNaN(numQuantity) && numQuantity > remainingQty && (
                  <p className="mt-1 text-[11px] font-medium text-[#FF3B30] dark:text-[#FF453A]">
                    Quantity cannot exceed remaining allocated quantity ({remainingQty}).
                  </p>
                )}
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="e.g. Salesperson returned unsold stock to Main Shop"
                  value={reason}
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-[#6E6E73] dark:text-[#86868B] leading-relaxed">
                    You are about to reverse <strong className="text-[#1D1D1F] dark:text-[#F5F5F7]">{numQuantity} unit(s)</strong> of <strong className="text-[#1D1D1F] dark:text-[#F5F5F7]">{allocation.product.name}</strong> from <strong className="text-[#1D1D1F] dark:text-[#F5F5F7]">{allocation.salesperson.name}</strong> back to <strong className="text-[#0071E3] dark:text-[#2997FF]">Main Shop</strong>.
                  </p>
