...
export const EditProductModal: React.FC<EditProductModalProps> = ({ product, isOpen, onClose, onSuccess }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto p-4 sm:p-6">
      ...
      <div className="relative my-8 flex w-full max-w-2xl flex-col rounded-2xl border border-[#D2D2D7] bg-white shadow-xl dark:border-[#38383A] dark:bg-[#1C1C1E]">
        ...
        <div className="flex items-start justify-between border-b border-[#E8E8ED] px-5 py-4 dark:border-[#2C2C2E]">
          <div>
            <h3 className="text-base font-semibold text-[#1D1D1F] dark:text-[#F5F5F7]">Edit Product</h3>
            <p className="mt-0.5 text-xs text-[#6E6E73] truncate max-w-sm">{product.name}</p>
          </div>
        ...
        <div className="overflow-y-auto px-5 py-4" style={{ maxHeight: 'calc(85vh - 112px)' }}>
          ...
          <ProductForm initialValues={product} isEdit onSubmit={handleSubmit} onCancel={onClose} isLoading={isLoading} />
        </div>
        ...
        <div className="flex items-center justify-end gap-2 border-t border-[#E8E8ED] px-5 py-3 dark:border-[#2C2C2E]">
          ...
          <Button variant="primary" size="sm" type="submit" form="product-form" isLoading={isLoading}>Save Changes</Button>
        </div>
      </div>
    </div>
  );
