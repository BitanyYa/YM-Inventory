...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const [productType, setProductType] = useState<ProductType>(
    initialValues?.productType || 'PHONE',
  );
  const [trackingType, setTrackingType] = useState<TrackingType>(
    initialValues?.trackingType || 'SERIALIZED',
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {/* Tracking Type Radio Choice (Create Only) */}
      {!isEdit && (
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-semibold uppercase tracking-wider text-slate-700 dark:text-slate-300">
            Stock Tracking Mode *
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            >
              ...
              <span className="mt-1 text-xs text-slate-500">
                Track individual items by IMEI / Serial Number (Phones, Laptops)
              </span>
            </label>
            ...
            >
              ...
              <span className="mt-1 text-xs text-slate-500">
                Track total item count only (Accessories, Chargers, Cases)
              </span>
            </label>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <textarea
          ...
          placeholder="Product details, specs, or notes..."
          value={description}
